...
export const Oasis_APIProvider = ({ children }) => {
  ...
  const api_getNFTsByPage = async (pageNumber) => {
    console.log("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
    try {
      const res = await axios({
        method: "GET",
        url: "http://localhost:3000/nft/page/" + pageNumber,
        withCredentials: true,
      });
      console.log(res.data.data.nft);
      if (res.data.status == "success") {
  ...
  const api_soldNFT = async (nftID) => {
    try {
      const resNFT = await axios({
        method: "PATCH",
        url: "http://localhost:3000/nft/sold",
        withCredentials: true,
        data: {
          id: nftID,
        },
  ...
  const api_getAllAccount = async () => {
    ...
    try {
      const res = await axios({
        method: "GET",
        url: "http://localhost:3000/account/all1",
        withCredentials: true,
      });
      console.log(res.data.data.account);
      if (res.data.status == "success") {
